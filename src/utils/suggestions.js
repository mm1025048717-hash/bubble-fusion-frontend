...
/**
 * 调用后端 API 获取建议
 * 优先使用简化代理端点 /api/fuse/suggest（不需要认证）
 * @param {string} textA 
 * @param {string} textB 
 * @param {string} prompt 
...
async function fetchSuggestions(textA, textB, prompt, options = {}) {
  try {
    // 优先使用简化代理端点（支持 DeepSeek 和阿里百炼）
    // 如果设置了 window.__BFL_API__，使用它；否则使用默认 API base URL
    const baseURL = (typeof window !== 'undefined' && window.__BFL_API__) 
      ? window.__BFL_API__ 
      : (import.meta.env.VITE_API_URL || 'http://localhost:3001/api').replace('/api', '');
    
    // 尝试使用简化代理端点 /api/fuse/suggest
    let response;
    try {
      response = await axios.post(`${baseURL}/api/fuse/suggest`, {
        a: textA,
        b: textB,
        context: {
          prompt: prompt || '',
          ...
          agent: options.agent || 'fusion',
          ...options
        }
      });
    } catch (proxyError) {
      // 如果简化代理失败，尝试使用完整后端 API（需要认证）
      console.warn('简化代理端点失败，尝试完整后端 API:', proxyError);
      response = await api.post('/ai/suggest', {
        a: textA,
        b: textB,
    ...
    // 处理响应数据格式
    let result = {
      title: data.title || `${textA} × ${textB}`,
      notes: data.notes || '',
      structured: data.structured || null,
      provider: data.provider || 'api'
    };
    
    // 如果返回的是 structured 数据，尝试从 structured 中提取 notes
    if (result.structured && result.structured.notes && Array.isArray(result.structured.notes)) {
      result.notes = result.structured.notes.join('\n');
    }
